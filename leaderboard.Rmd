...
```{r}
valueBox(value = format(Sys.time(), '%d %B, %Y'), icon = "fa-calendar")
```
...
```{r}
valueBox(value = n_distinct(contest_recs$user), icon = "fa-users")
```

### Refuges/Hatcheries with entries

```{r}
valueBox(n_distinct(contest_recs$orgname), icon = "fa-pagelines")
```
